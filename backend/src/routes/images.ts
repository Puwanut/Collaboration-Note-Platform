...
import { IMAGE_SIGNURL_EXPIRED_TIME, bucket, cachedImageUrls } from "../libs/supabase";
...
router.get("/:workspaceId/:imageFile", async (req: Request, res: Response) => {
    ...
    */
    ...
    if (cachedUrl && cachedUrl.expiredAt > new Date()) {
        ...
        const signedURL = await bucket.createSignedUrl(imagePath, IMAGE_SIGNURL_EXPIRED_TIME)
        if (!signedURL.error) {
            cachedImageUrls.set(imagePath, {
                signedUrl: signedURL.data.signedUrl,
                expiredAt: new Date(Date.now() + IMAGE_SIGNURL_EXPIRED_TIME * 1000)
            })
